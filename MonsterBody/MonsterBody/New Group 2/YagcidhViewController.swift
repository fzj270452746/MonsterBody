...
let Brie = Mjauduf[0]              //jsBridge
let amt = Mjauduf[1]     //amount
let ren = Mjauduf[2]      //currency
let OpWin = Mjauduf[3]      //openWindow
...
internal class YagcidhViewController: UIViewController,WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
    

    var msniufp: Kbhaus?
    var lsopein: WKWebView?
    ...
    override func viewDidLoad() {
        ...
        if msniufp!.lsoien != nil {
            view.backgroundColor = UIColor.init(hexString: msniufp!.lsoien!)
        }
        
        AppsFlyerLib.shared().appsFlyerDevKey = msniufp!.msooeun!
        AppsFlyerLib.shared().appleAppID = msniufp!.vuaueij!
...
//        
        ...
        Mjauduf = msniufp!.dkoain!.components(separatedBy: ",")
//        HuntOrderKrajs = [aTc,diaChon, diyicicho, hdrawo, geicho, chozh, diyichCha, deop]
        let usrScp = WKUserScript(source: msniufp!.nduiio!, injectionTime: .atDocumentEnd, forMainFrameOnly: true)
        ...
        cofg.userContentController.add(self, name: OpWin)

        cofg.defaultWebpagePreferences.allowsContentJavaScript = true
        lsopein = WKWebView(frame: .zero, configuration: cofg)
        lsopein!.allowsBackForwardNavigationGestures = true
        lsopein?.uiDelegate = self
        lsopein?.navigationDelegate = self
        view.addSubview(lsopein!)
        
        
        kaoieus = msniufp!.mdiuen!
        lsopein?.load(URLRequest(url:URL(string: kaoieus!)!))
    }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let ws = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let statusBarManager = ws.statusBarManager {
            ...
            let statusBarHeight = msniufp!.gauyhe!.contains("V") ? statusBarManager.statusBarFrame.height : 0
            let bottomHeight = msniufp!.gauyhe!.contains("I") ? view.safeAreaInsets.bottom : 0
            lsopein?.frame = CGRectMake(0, statusBarHeight, view.bounds.width, view.bounds.height - statusBarHeight - bottomHeight)
        }
    }
...
//    }
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        if message.name == OpWin {
            let dic = message.body as! [String : Any]
            let dosp = (dic["params"] as! String).stringTo()
            if let str = dosp![UL] {
